...
export class DataTable extends React.Component {
  ...
  handleSort = sortKey => {
    let {sort} = this.state;
    let nextSort = sort === null ? {orderBy: sortKey, orderAsc: true} : sort.orderAsc === true ? {
      orderBy: sortKey,
      orderAsc: false
    } : null;
  ...
  render() {
    let {className, columns, rowLinkTo, pageSize = 50} = this.props;
            ...
            <tr>
              {columns.map((each, i) => (
                <th key={i}
                    ...
                    className={each.headerClassName}
                >
                  {each.label}
